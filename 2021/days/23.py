...
def hallway_clear(i, j, hallway, room_size):
    ...
    if j <= 3 and i >= 3 * room_size and hallway[4] != ".":
        return False
    if j >= len(hallway) - 2 and i < 3 * room_size and hallway[4] != ".":
        return False
    if j >= len(hallway) - 3 and i < 2 * room_size and hallway[3] != ".":
        return False
    if j >= len(hallway) - 4 and i < room_size and hallway[2] != ".":
        return False
    return True


def penalty(i, j, c, room_size):
    room = (i // room_size) * room_size + room_size
...
@cache
def dfs(rooms, hallway, room_size, kkk):
    # print(rooms, hallway)
    ...
    while changes:
        ...
        for i, x in enumerate(hallway):
            ...
            if not hallway_clear(j, i, hallway, room_size):
                continue
            for k in reversed(range(j, j+room_size)):
                if all(rooms[l] == "." for l in range(j, k+1)):
    ...
    for i, x in enumerate(rooms):
        if x == ".":
            continue
        if i % room_size > 0 and any(rooms[i-k-1] != "." for k in range(i % room_size)):
            continue
        ...
        for j, y in enumerate(hallway):
            ...
            if not hallway_clear(i, j, hallway, room_size):
                continue
            kkk += dis + penalty(i, j, x, room_size)
            sol = min(sol, dfs(rooms[:i] + "." + rooms[i+1:], hallway[:j] + x + hallway[j + 1:], room_size, kkk)
                      + dis + penalty(i, j, x, room_size))
    return sol
...
def part_two(data: str) -> int:
    ...
    return dfs(room0 + room1 + room2 + room3, ".......", 4, 0)
